...
{
    // Базовый класс полигональной фигуры: хранит точки, грани, материалы и аффинные преобразования
    public class Figure
    {
        public static float EPS = 0.0001f;              // допуск для сравнения с нулём
        ...
        // материалы для стен комнаты (используются при isRoom)
        ...
        public Material figure_material;            // материал самой фигуры

        public Figure() { }

        // Копирующий конструктор: точки и стороны копируются в новый объект
        public Figure(Figure f)
        {
            ...
            foreach (Side s in f.sides)
            {
                ...
                sides.Last().host = this; // переназначаем владельца
            }
        }

        // Пересечение луча с треугольником (алгоритм Möller–Trumbore)
        public bool ray_intersects_triangle(Ray r, Point3D p0, Point3D p1, Point3D p2, out float intersect)
        {
            ...
            if (v < 0 || u + v > 1)
                return false;
            // На этом этапе вычисляем t — расстояние до точки пересечения
            ...
            else      // линия пересекает треугольник, но не луч
                return false;
        }

        // Пересечение луча с фигурой: проверяем все грани (квадраты как два треугольника)
        public virtual bool figure_intersection(Ray r, out float intersect, out Point3D normal)
        {
            intersect = 0; 
            normal = null;
            Side sd = null;        // пересечённая грань
            int fm = -1;           // номер стены комнаты, которую пересёк луч
            ...
            for (int i = 0; i < sides.Count; ++i)
            {
                ...
                {
                    // проверяем два треугольника, на которые разбивается квадрат
            ...
            if (intersect != 0)
            {
                normal = Side.norm(sd); // нормаль пересечённой грани
                if (isRoom)
                    switch (fm)
                    {
                        case 0:
                ...
                // Цвет материала берём из пера грани
        ...
        // Представление точек фигуры в виде матрицы Nx4 (x, y, z, 1)
        ...
        // Применение матрицы преобразования (делим на w для перехода из однородных координат)
        ...
        // Центр фигуры как среднее всех точек
        ...
        public virtual void set_pen(Pen dw)
        {
            foreach (Side s in sides)
                s.drawing_pen = dw; // одинаковый цвет для всех граней
        }
        ...
        // ----------------------------- СТАТИЧЕСКИЙ СЕРВЕР ДЛЯ ПРЕОБРАЗОВАНИЙ --------------------------------
        ...
        // ------------------------ СТАТИЧНЫЕ ФАБРИЧНЫЕ ФИГУРЫ -----------------------------
        // Построение гексаэдра (куба) размера sz
        static public Figure GetHexahedron(float sz)
        {
            ...
            // Грани куба как стороны с индексами точек
            Side s = new Side(res);
